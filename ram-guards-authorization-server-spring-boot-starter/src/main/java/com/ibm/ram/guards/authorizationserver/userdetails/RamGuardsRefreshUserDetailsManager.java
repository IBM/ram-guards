...
@Component
@Slf4j
@Qualifier("refreshUserDetailsService")
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class RamGuardsRefreshUserDetailsManager implements RamGuardsUserDetailsService {

    private final @NonNull RamGuardsUserDetailsServiceProvider ramGuardsUserDetailsServiceProvider;

    @Override
    @CachePut(cacheNames = AUTHORIZATION_RAM_SERVER_CACHE_NAME,
            key = "#p0.redisKey",
    ...
    public UserDetails loadUserByRamGuardsUser(RamGuardsUser ramGuardsUser) throws UsernameNotFoundException {
        log.info("Refresh user: {}'s UserDetails without caching", ramGuardsUser);
        try {
